...
{
    public class EmpSPDDAL
    {
        public void Insert(Hashtable parameters, ref int iNextID)
        {
            DBWrapper dbw = DBWrapper.GetSqlClientWrapper();
            dbw.ConnectionString = Shared.ConnectionString;
            if (!dbw.ExecuteSP("sp_EmpSPD_insert", parameters, ref iNextID))
            {
                throw new Exception("Fail to execute sp_EmpSPD_insert", new Exception(dbw.DBErrorMessage));
            }
        }
        ...
        public void Update(Hashtable parameters)
        {
            DBWrapper dbw = DBWrapper.GetSqlClientWrapper();
            dbw.ConnectionString = Shared.ConnectionString;
            if (!dbw.ExecuteSP("sp_EmpSPD_update", parameters))
            {
                throw new Exception("Fail to execute sp_EmpSPD_update", new Exception(dbw.DBErrorMessage));
            }
        }
        ...
        public void Delete(Hashtable parameters)
        {
            DBWrapper dbw = DBWrapper.GetSqlClientWrapper();
            dbw.ConnectionString = Shared.ConnectionString;
            if (!dbw.ExecuteSP("sp_EmpSPD_delete", parameters))
            {
                throw new Exception("Fail to execute sp_EmpSPD_delete", new Exception(dbw.DBErrorMessage));
            }
        }
        ...
        public DataTable GetRows(Hashtable parameters)
        {
            ...
            dbw.ConnectionString = Shared.ConnectionString;
            if (!dbw.ExecuteSP("sp_EmpSPD_getrows", parameters, ds))
            {
                throw new Exception("Fail to execute sp_EmpSPD_getrows", new Exception(dbw.DBErrorMessage));
            }
            else
            {
                if (ds.Tables.Count <= 0)
                    throw new Exception("Fail to sp_EmpSPD_getrows. No row found.");
                else
                    return ds.Tables[0];
            }
        }
