...
{
    public class OJKMasterDAL
    {
        public DataTable GetRowsForSubMenuOJKMaster(Hashtable parameters)
        {
            DBWrapper dbw = DBWrapper.GetSqlClientWrapper();
            DataSet ds = new DataSet();
            dbw.ConnectionString = Shared.ConnectionString;
            if (!dbw.ExecuteSP("sp_ojk_master_getrows_forsubmenu", parameters, ds))
            {
                throw new Exception("Fail to execute sp_ojk_master_getrows_forsubmenu", new Exception(dbw.DBErrorMessage));
            }
            else
            {
                if (ds.Tables.Count <= 0)
                    throw new Exception("Fail to sp_ojk_master_getrows_forsubmenu. No row found.");
                else
                    return ds.Tables[0];
            }
        }
        ...
        //public DataTable GetRowsForCollMemo(Hashtable parameters)
        //{
        //    DBWrapper dbw = DBWrapper.GetSqlClientWrapper();
        //    DataSet ds = new DataSet();
        //    dbw.ConnectionString = Shared.ConnectionString;
        //    if (!dbw.ExecuteSP("sp_master_report_getrows_collmemo", parameters, ds))
        //    {
        //        throw new Exception("Fail to execute sp_master_report_getrows_collmemo", new Exception(dbw.DBErrorMessage));
        //    }
        //    else
        //    {
        //        if (ds.Tables.Count <= 0)
        //            throw new Exception("Fail to sp_master_report_getrows_collmemo. No row found.");
        //        else
        //            return ds.Tables[0];
        //    }
        //}
